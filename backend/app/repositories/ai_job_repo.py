...
class AIJobRepository(BaseRepository):
    ...
    async def create_job(self, user_id: str, asset_id: str, job_type: str) -> str:
        doc = {
            "_id": str(ObjectId()),
            "user_id": user_id,
            "asset_id": asset_id,
    ...
    async def update_status(self, job_id: str, status: str, result_id: Optional[str] = None) -> bool:
        update_data = {
            "status": status,
